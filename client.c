...
{
    DEBUGF("[client] terminating helper (pid %d)", helper_pid);

    if (kill(helper_pid, SIGTERM) == -1 && errno != ESRCH) {
        ERRORF("failed to terminate helper: %s", strerror(errno));
        return;
    }

    DEBUG("[client] waiting for helper");

    pid_t result;
    do {
        result = waitpid(helper_pid, NULL, 0);
    } while (result == -1 && errno == EINTR);

    DEBUG("[client] helper terminated");
}
...
{
    DEBUGF("[client] terminating command (pid %d)", command_pid);

    if (kill(command_pid, SIGTERM) == -1 && errno != ESRCH) {
        ERRORF("failed to terminate command: %s", strerror(errno));
        return;
    }

    DEBUG("[client] waiting for command");

    pid_t result;
    do {
        result = waitpid(command_pid, NULL, 0);
    } while (result == -1 && errno == EINTR);

    DEBUG("[client] command terminated");
}
